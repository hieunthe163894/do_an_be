...
const getClassWorkByStudent = async ({userId, type}) => {
    try {
        ...
        const classWorkWithGrades = classWorks.map(classwork => {
            const isActive = currentDate >= classwork.startDate && currentDate <= classwork.dueDate;
            const submissionData = submissionMap.get(classwork._id.toString()) || { grade: null, gradingCriteria: null };
            return {
                _id: classwork._id,
                ...
                isActive: isActive,
                grade: submissionData.grade && submissionData.grade.grade !== undefined ? submissionData.grade.grade : null,
                gradingCriteriaSubmission: submissionData.grade && submissionData.grade.gradingCriteria !== undefined ? submissionData.grade.gradingCriteria : [],
            };
        });
